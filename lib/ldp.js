...
class LDP {
  ...
  post (host, containerPath, stream, { container, slug, extension }, callback) {
    ...
    ldp.getAvailablePath(host, containerPath, { slug, extension }).then(resourcePath => {
      ...
      ldp.putStream(host, resourcePath, stream, function (err) {
        if (err) callback(err)
  ...
  putGraph (graph, uri, contentType = DEFAULT_CONTENT_TYPE) {
    return new Promise((resolve, reject) => {
      ...
      serialize(graph, uri, contentType, (error, content) => {
        ...
        this.putStream(hostname, path, stream, (error) => {
          if (error) { return reject(error) }
  ...
  _put (host, resourcePath, callback, resourceHandler) {
    const ldp = this
    ...
    mkdirp(dirName, (err) => {
      ...
      resourceHandler(filePath)
    })
  }

  putText (host, resourcePath, text, callback) {
    this._put(host, resourcePath, callback, (filePath) => {
      try {
  ...
  putStream (host, resourcePath, stream, callback) {
    this._put(host, resourcePath, callback, (filePath) => {
